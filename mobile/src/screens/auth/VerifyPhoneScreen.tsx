...
/**
 * Two-step screen: collect a Brazilian phone number → request OTP → enter
 * the 6-digit code. Once the backend confirms the code, the user advances to
 * VerifyCpf. Validation runs on every keystroke so the CTA is only enabled
 * when the input would actually pass the backend's regex.
 */
export function VerifyPhoneScreen({ navigation }: Props) {
  ...
  const onVerifyCode = async () => {
    ...
    try {
      ...
      // RootNavigator automatically shows VerifyCpf when phoneE164 is set.
    } catch {
