...
type Props = NativeStackScreenProps<AuthStackParamList, 'VerifyCpf'>;

/**
 * Realtime CPF input. Mask is applied on each keystroke; the mod-11 check
 * runs the moment 11 digits are present so the CTA flips to enabled exactly
 * when the input is valid.
 */
export function VerifyCpfScreen({ navigation }: Props) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      // RootNavigator automatically shows LgpdConsent when cpf is set.
    } catch {
        ...
        <View className="px-6 pb-12">
          ...
          >
            {busy ? (
              ...
              <Text className="text-arena-fundo font-bold text-base">Continuar</Text>
            )}
          </Pressable>
        </View>
      </KeyboardAvoidingView>
